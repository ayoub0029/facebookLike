...
<p dangerouslySetInnerHTML={{ __html: UserIcon }} />
</a> */}
...
function SearchData(data , FetchUsers = false , FetchGroups = false) {
    const Res = []
    console.log(FetchUsers);
    console.log(FetchGroups);
    if (FetchUsers) {
        UsersSearch(data).then(users => users?Res.push(...users) : null)
    }
    if (FetchGroups) {
        GroupSearch(data).then(groups => groups?Res.push(...groups) : null)
    }
    console.log(Res);
    return Res
}
...
export function SearchInput({ FetchUsers = false, FetchGroups = false }) {
    const handleSearch = useCallback(
        debounce((element) => {
            SearchData(element.value , FetchUsers , FetchGroups) 
        }, 200),[]
    );
    return (
        <>
            <div class="searchBar">
                <input onInput={(e) => handleSearch(e.target)} type="text" placeholder="&#xF002; Search..." />
