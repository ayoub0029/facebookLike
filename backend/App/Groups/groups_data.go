  ...
  CASE WHEN EXISTS (
    ...
    WHERE g.id = ? AND (
      ...
      OR EXISTS (
        ...
        WHERE gm.group_id = g.id AND gm.user_id = ? AND gm.status = 'accept'
      )
    )
  )
  THEN ? ELSE 0 END AS id;`
	res, err := d.SelectOneRow(query, groupId, userId, userId, userId)
	if err != nil {
		return false
	} else {
		id := -1
...
func getPeopleToInvite(userID, groupID, page int) []profiles.Profile {
	query := `SELECT u.id, u.first_name, u.last_name, u.avatar
			...
			WHERE g.id = ?) GROUP BY u.id LIMIT 5 OFFSET ?;`
